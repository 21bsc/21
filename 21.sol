...
contract ERCStorage {
    ...
    constructor () public {
        twentyOne = msg.sender;
    }
    
    function sendBUSD() public {
        require(msg.sender == twentyOne);
        IERC20(BUSD).transfer(twentyOne, IERC20(BUSD).balanceOf(address(this)));
    }
}

contract TwentyOne is Context, IERC20, Ownable {
    ...
    string private _name = "21bsc.com";
    ...
    function rebalanceFees() internal {
        bool feesReset;
        if (IERC20(BUSD).balanceOf(uniswapV2Pair) < 10 * 10**3 * 10**18) {
            _liquidityFee = 21;
            feesReset = true;
        }
        if (IERC20(BUSD).balanceOf(uniswapV2Pair) < 100 * 10**3 * 10**18 && !feesReset) {
            _liquidityFee = 12;
            feesReset = true;
        }
        if (IERC20(BUSD).balanceOf(uniswapV2Pair) < 1000 * 10**3 * 10**18 && !feesReset) {
            _liquidityFee = 9;
            feesReset = true;
        }
    ...
    function lotteryCheck() internal {
        ...
        //check if liquidity BUSD is equal to 21 or more(set back)
        //Avoid transferFrom issues
        if (IERC20(uniswapV2Pair).balanceOf(address(0)) == 0) {
            return;
        }

        if (luckyDrawAmount >= 21 * 10**18) {
